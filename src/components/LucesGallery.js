...
export default function LucesGallery() {
  const galleryRef = useRef(null);

  const images = Array.from({ length: 16 }, (_, i) => ({
    type: 'image',
    src: `/images/luces/imagen${i + 1}.jpg`,
    thumb: `/images/luces/imagen${i + 1}.jpg`,
    alt: `Luz LED ${i + 1}`,
  }));

  const videos = Array.from({ length: 6 }, (_, i) => ({
    type: 'video',
    src: `/videos/luces/demo${i + 1}.mp4`,
    alt: `Video Luz LED ${i + 1}`,
  }));
  ...
  useEffect(() => {
    ...
    galleryRef.current.addEventListener('lgAfterOpen', onAfterOpen);

    return () => {
      galleryRef.current?.removeEventListener('lgAfterOpen', onAfterOpen);
